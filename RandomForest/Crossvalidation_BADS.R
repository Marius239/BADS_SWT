...
test.data<- read.csv2("TestDataSet.csv")
train.data<- read.csv2("TrainingDataSet.csv")
...
for(i in 1:k){
  ...
  fit= randomForest(x=cv.train[,-c(1,2)], y=as.factor(cv.train[,2]))      #include your prediction model
  prediction=predict(fit, newdata=cv.test[,-c(1,2)], type="prob")[,2]
  
  err.vect[i]=roc.area(cv.test[,2], prediction)$A
  pred<-prediction(prediction,as.factor(cv.test[,2]) )
