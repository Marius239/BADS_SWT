...
load("TrainingDataMedian40Var.RData")
train.data <- trainingSetMedian
...
rf <- function(x, trees){
  ...
  churn_num_train <- as.integer(as.numeric(cv.train$churn) - 1)
  
  fit = gbm.fit(x = cv.train[,which(names(cv.train) != "churn")], 
                y = churn_num_train, 
                n.trees = trees, verbose=FALSE, shrinkage=0.005, 
                interaction.depth=20, n.minobsinnode=5, distribution="bernoulli")   
  
  prediction = predict(fit, newdata = cv.test[,which(names(cv.train) != "churn")], n.trees = trees ,type="response")[,2]
...
filename <- paste("Results_GBM/GBM_", t, "Tree_", nvar,"Vars_Phone_Median.RData", sep = "")
